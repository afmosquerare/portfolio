...
@Component({
  ...
  template: `
    <dialog class="modal" [class.modal-open]="isOpen()">
      <div class="modal-box w-[calc(100%-2rem)] max-w-4xl max-h-[calc(100vh-4rem)] bg-base-200 border border-base-content/10 shadow-2xl relative p-0 overflow-hidden flex flex-col rounded-md">
        <button (click)="isOpen.set(false)"
          class="btn-md cursor-pointer p-0 btn-square  absolute right-3 top-3 sm:right-4 sm:top-4 z-20 bg-black/50 text-base-content hover:bg-black/70 backdrop-blur-sm border-none">✕</button>
        ...
        <div class="p-6 md:p-12">
          <h3 class="font-display font-bold text-2xl md:text-4xl mb-6">{{ langService.currentLang() === 'en' ? project().titleEn : project().titleEs }}</h3>
